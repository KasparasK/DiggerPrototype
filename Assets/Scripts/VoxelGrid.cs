...
public class VoxelGrid : MonoBehaviour
{
    ...
    public void Initialize(int resolution, float size, float maxFeatureAngle, int matID,bool isLast,Action initCallback)
    {
        ...
        surface.Initialize(resolution, matID, isLast, InitCallback);

        wall = Instantiate(wallPrefab) as VoxelGridWall;
        wall.transform.parent = transform;
        wall.transform.localPosition = Vector3.zero;
        wall.Initialize(resolution, matID, InitCallback);
    }
    ...
    public void Apply(VoxelStencil stencil, ref int activeVoxelsCount)
    {
        ...
        for (int y = yStart; y <= yEnd; y++)
        {
            ...
            {
                stencil.Apply(voxels[i], ref activeVoxelsCount);
            }
        }
